...
//                        ./${projectName}" // 最後のパラメータはDockerfileのあるディレクトリ
        ...
        stage("Make ConnectionPool") {
            ...
            steps {
                ...
                sh "docker exec -it javatraining-99 asadmin --user admin --passwordfile passwordFile create-jdbc-resource --enabled=true --poolName=mysql-pool --target=domain jdbc/tx"
                sh "docker exec -it javatraining-99 asadmin --user admin --passwordfile passwordFile create-resource-ref --enabled=true --target=server jdbc/tx"
                }
            }
        }
        stage("Data Setup") {
