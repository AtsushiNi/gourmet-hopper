...
//                        ./${projectName}" // 最後のパラメータはDockerfileのあるディレクトリ
        ...
        stage("Make ConnectionPool") {
            ...
            steps {
                ...
                sh "docker exec javatraining-99 asadmin --user admin --passwordfile passwordFile create-jdbc-resource --enabled=true --poolName=mysql-pool --target=domain jdbc/tx"
                sh "docker exec javatraining-99 asadmin --user admin --passwordfile passwordFile create-resource-ref --enabled=true --target=server jdbc/tx"
                }
            }
        }
        stage("Data Setup") {
            ...
            steps {
                script {
                sh "scp -i javatraining_mysql5.7_rsa java_training_ddl.sql ec2-user@3.112.209.238:~/"
                sh "ssh -i javatraining_mysql5.7_rsa ec2-user@3.112.209.238 mysql -uroot -p password < java_training_ddl.sql"
                }
            }
        }
    }
